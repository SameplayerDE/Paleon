...
namespace Paleon
{
    public class GotoPlantTask : Task
    {
        ...
        public GotoPlantTask(FarmArea farmland)
        {
            this.farmland = farmland;
        }

        public GotoPlantTask(Tile tile)
        {
            targetTile = tile;
        }
        ...
        public override bool Check(SettlerCmp settler)
        {
            ...
            else
            {
                for (int i = 0; i < farmland.Tiles.Count; i++)
                {
                    ...
                    if (settler.Pathfinder.IsPathAvailable(tile, false) && tile.GroundTopType == GroundTopType.FARMLAND && tile.Entity == null)
                    {
                ...
                case MovementState.Success:
                    {
                        ...
                        {
                            Oat oat = new Oat(targetTile, GameplayScene.Instance);

                            GameplayScene.AddEntity(oat, targetTile);
